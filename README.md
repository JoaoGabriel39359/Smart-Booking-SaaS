...
Banco de Dados: PostgreSQL com SQLAlchemy ORM (Hospedado via Supabase)

Automação de WhatsApp: Evolution API (Integração via Webhooks e Rest)

Calendário: Google Calendar API v3 (OAuth2 com Refresh Token)
...
OAuth2 Flow: O professor autoriza o app uma única vez.

Refresh Token: O sistema armazena o token de atualização para manter o acesso vitalício.

Background Tasks: Jobs assíncronos verificam o banco de dados e disparam lembretes via WhatsApp sem travar a aplicação.

🔧 Configuração do Ambiente
Para rodar o projeto, você precisará configurar as seguintes variáveis no seu arquivo .env:
